...
function setRandomHeroHeadline() {
    ...
    const previous = headlineEl.dataset.heroHeadlineIndex
        ? Number(headlineEl.dataset.heroHeadlineIndex)
        : -1;

    let nextIndex = Math.floor(Math.random() * options.length);
    if (options.length > 1 && nextIndex === previous) {
        nextIndex = (nextIndex + 1) % options.length;
    }

    headlineEl.dataset.heroHeadlineIndex = String(nextIndex);
    headlineEl.innerHTML = options[nextIndex];
}

function initTickerContinuousLoop() {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    setInterval(setRandomHeroHeadline, 6000);
    initTickerContinuousLoop();
    loadHomepage();
});